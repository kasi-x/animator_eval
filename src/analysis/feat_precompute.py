"""DuckDB-native feature pre-computation (§4.4 pipeline Phase 1.5 migration).

Replaces the SQLite compute_feat_* functions in database.py for the three
tables that only require credits (silver.duckdb → gold.duckdb):
...
compute_feat_studio_affiliation() remains in database.py until anime_studios /
studios are added to silver.duckdb (§4.3 Parquet card scope).
"""
...
from src.analysis.gold_writer import gold_connect_write
