...
SOURCE_TABLE_TO_SILVER: dict[tuple[str, str], _MappingValue] = {
    ...
    ("anilist", "anime"): ("anime", "id LIKE 'anilist:%'", "id"),
    ("anilist", "persons"): ("persons", "id LIKE 'anilist:%'", "id"),
    ...
    ("anilist", "characters"): ("characters", "id LIKE 'anilist:%'", "id"),
    ...
    ("anilist", "studios"): ("studios", "id LIKE 'anilist:%'", "id"),
    (
        "anilist",
        "anime_studios",
    ): None,  # merged with other sources, not directly filterable
    ...
    ("ann", "credits"): ("credits", "evidence_source = 'ann'", None),
    ...
    ("bangumi", "subjects"): ("anime", "id LIKE 'bgm:%'", "id"),
    ("bangumi", "persons"): ("persons", "id LIKE 'bgm:%'", "id"),
    ("bangumi", "subject_persons"): ("credits", "evidence_source = 'bangumi'", None),
    ("bangumi", "characters"): (
        "characters",
        "id LIKE 'bgm:%' OR id LIKE 'bgm:%'",
        "id",
    ),
    ...
    (
        "bangumi",
        "person_characters",
    ): None,  # character voice-actor relationship, not loaded
    # ── Keyframe ───────────────────────────────────────────────────────────
    ("keyframe", "anime"): ("anime", "id LIKE 'keyframe:%'", "id"),
    ("keyframe", "persons"): ("persons", "id LIKE 'keyframe:%'", "id"),
    ...
    ("keyframe", "credits"): ("credits", "evidence_source = 'keyframe'", None),
    (
        "keyframe",
        "person_credits",
    ): None,  # alternative credits path, merged into credits
    ("keyframe", "anime_studios"): (
        "anime_studios",
        None,
        None,
    ),  # all rows from keyframe
    # keyframe/studios_master → SILVER studios WHERE id LIKE 'keyframe:s%'
    # (keyframe:n:% IDs originate from anime_studios names, not studios_master)
    ("keyframe", "studios_master"): ("studios", "id LIKE 'keyframe:s%'", "studio_id"),
    ("keyframe", "person_jobs"): ("person_jobs", "source = 'keyframe'", None),
    ...
    # ── MAL ────────────────────────────────────────────────────────────────
    ("mal", "anime"): ("anime", "id LIKE 'mal:%'", None),
    ("mal", "staff_credits"): ("credits", "evidence_source = 'mal'", None),
    ("mal", "va_credits"): ("character_voice_actors", "source = 'mal'", None),
    ...
    ("mediaarts", "anime"): ("anime", "id LIKE 'madb:%'", "id"),
    ("mediaarts", "persons"): ("persons", "id LIKE 'madb:%'", "id"),
    ("mediaarts", "credits"): ("credits", "evidence_source = 'mediaarts'", None),
    ...
    ("seesaawiki", "anime"): ("anime", "id LIKE 'seesaa:%'", "id"),
    ("seesaawiki", "persons"): ("persons", "id LIKE 'seesaa:%'", None),
    ("seesaawiki", "credits"): ("credits", "evidence_source = 'seesaawiki'", None),
    ("seesaawiki", "studios"): ("studios", "id LIKE 'seesaa:%'", None),
...
def check(
    ...
    if not tables:
        logger.warning(
            "silver_completeness.check: no BRONZE tables found",
        ...
        for source, table in tables:
            ...
            if mapping == "UNKNOWN":
                ...
                bronze_rows, b_err = _count_bronze_rows(
                    bronze_conn, bronze_root, source, table
                )
                rows.append(
                    CoverageRow(
                        source=source,
            ...
            silver_rows, s_err = _count_silver_rows(
                silver_conn, silver_table, silver_filter
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--out", type=Path, default=Path("result/audit/silver_completeness.md")
    )
