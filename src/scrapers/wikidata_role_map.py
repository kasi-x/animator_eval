...
- P58   "screenplay"    → Role.SCREENPLAY       (Wikidata label is "screenwriter",
                                                 ROLE_MAP keys on "screenplay")
- P3174 "art director"  → Role.BACKGROUND_ART

Skipped pending further work:

- P1040 ("film editor") — `Role.EDITING` exists but neither "editing" nor
  "film editor" is currently in `ROLE_MAP`, so a credit built from P1040
  would fall back to `Role.SPECIAL`. Add the English key to `ROLE_MAP`
  first, then re-enable here.
- P10800 (purportedly "animation director") — property ID unverified on
  wikidata.org. "animation director" is already a valid ROLE_MAP key, so
  the value is ready as soon as the property ID is confirmed.
"""
