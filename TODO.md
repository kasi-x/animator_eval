...
| 優先度 | カテゴリ | 内容 |
|--------|---------|------|
| ✅ Done | Resolved cluster | madb parent link (`708669d`) + tmdb homonym guard (`f0d4547`) + audit script (`f0d4547`) + canonical_id collision (`686e161`) + Phase 2b keyframe prefix統一 (`d8bd282` / `09a13df`)、32+ tests pass、docs sync完了 (2026-05-15) |
...
| ✅ Done | 業界構造観察 | 制作委員会 centrality (26/01) / 国際協業 (26/02) / studio pipeline strength (26/03) 3 cards 完了 → `TASK_CARDS/26_industry_structure/`。26/02 は nationality 流入路欠陥で実データ動作には `35/01` 完了必要 |
...
| `15/04_o4_foreign_talent` | O4 | 海外人材 (国籍別 FE / 役職進行) | nationality カバレッジ | 🟡 | ✅ 実装完了 (2026-05-15、27 tests pass、Method 充足)、ただし nationality 入力 0% で実データ動作には `35/01` 完了必要 |
...
| `15/x_cross_cutting` | — | brief マッピング / 新 audience / lint_vocab / method_note template | 個別 O カードと並行可 | 🟡 | ✅ 完了 (2026-05-13、commit `27a1cad`、docs/report_cross_cutting.md + lint_vocab CONTEXTUAL_BIGRAMS L97-142 + section_builder METHOD_NOTE_TEMPLATES 8 手法 + forbidden_vocab_exceptions.yaml) |
...
| Card | 内容 | 経路 |
|------|------|------|
| `01_committee_influence` | ✅ 完了 (2026-05-15、commit `d8dab3e`) bipartite + eigenvector centrality + HHI、24 tests、coverage 1.7% 警告経路 | publication / 政策 |
| `02_international_collab` | ✅ 完了 (2026-05-15、41 tests pass、`international_collab.py` 893 + `structure_international.py` 853 + Louvain + null model permutation)。実データ動作は `35/01` nationality 流入路修復後 | business / publication |
| `03_studio_pipeline_strength` | ✅ 完了 (commit `f610ed2`) young_theta_growth/mid_retention/key_person/bus_factor + 16 tests | business (投資家・HR) |
...
| Card | 内容 |
|------|------|
| `01_audit_existing_reports` | ✅ 完了 (2026-05-15、49 files / 2 violations fixed / `audit_reports_labor_first.py` + `reports/audit_2026_05_15.json`) |
| `02_brief_restructure` | ✅ 完了 (commit `b454ead`) labor-first restructure (Workers brand) |
| `03_sns_outreach_layer` | ✅ 完了 (commit `8f69ac7`) X / note short-form auto-generator |
| `04_b2c_individual_view` | ✅ 完了 (2026-05-15、merge `91f70ae`) labor-first 個人 view、25 tests、opt-out / CI / IV decompose |
