...
| 🟢 Strategy | 拡張目的レポート | PROJECT.md §6.14 の 8 種拡張目的レポート → `TASK_CARDS/15_extension_reports/` (8 カード + 横断タスク 1)。優先順位: O3 → O1 → O2 → O4 → O6 → O8 → O7 → O5 |
...
## SECTION 15: gender enrichment scraper (新規 2026-05-02)

`15_extension_reports/02_o1_gender_ceiling` の Pre-condition で発覚: SILVER `persons.gender` null 率 95.4% (259,684 中 247,038)、閾値 70% を大幅超過し O1 実装ブロック。

候補 source:
- AniList GraphQL `Staff.gender` 拡張 (現行未取得、`12.1` SILVER ローダー実装と同時可能)
- MAL/Jikan `/v4/people/{id}` の gender (現行 BRONZE 未統合、`12.3` Card 05 全件 scrape 完了後に統合経路追加)
- gender-guesser ライブラリ (名前推定、注: false positive 多、補助のみ)
- ANN bio HTML 内 pronoun 抽出 (NLP)

実施は **scraper 系** タスク。本セクション起票のみ、実装は別途 Opus 設計レビュー後に投入。

依存: 完了後に `15_extension_reports/02_o1_gender_ceiling` 再起動。

---

## SECTION 14: 拡張目的レポート群 → `TASK_CARDS/15_extension_reports/`

PROJECT.md §6.14 で宣言した 8 種の拡張目的レポートを 8 カード + 横断タスクに分割。詳細は各カード参照。

### カード一覧 (実施推奨順)

| ID | 拡張目的 | 内容 | 依存 | Priority |
|----|--------|------|------|---------|
| `15/01_o3_ip_dependency` | O3 | IP 人的依存リスク (counterfactual + null model) | 既存 SILVER のみ | 🟠 |
| `15/02_o1_gender_ceiling` | O1 | ジェンダー天井効果 (Cox + ego-net + DID) | gender カバレッジ確認 | 🟠 |
| `15/03_o2_mid_management` | O2 | 中堅枯渇 (KM + スタジオ別 blockage) | 既存 SILVER のみ | 🟠 |
| `15/04_o4_foreign_talent` | O4 | 海外人材 (国籍別 FE / 役職進行) | `14_silver_extend` 完了 | 🟡 |
| `15/05_o6_cross_border` | O6 | 国際共同制作 (PageRank / community) | `04_o4` 完了 | 🟡 |
| `15/06_o8_soft_power` | O8 | ソフトパワー指標 | 海外配信メタ (Card 16 / 別経路) | 🟢 |
| `15/07_o7_historical` | O7 | 失われたクレジット復元 | schema 変更 (`confidence_tier`) | 🟢 |
| `15/08_o5_education` | O5 | 教育機関キャリア追跡 | 出身校データ取得経路 (現状不在) | 🟢 |
| `15/x_cross_cutting` | — | brief マッピング / 新 audience / lint_vocab / method_note template | 個別 O カードと並行可 | 🟡 |
...
短期 (parser 拡張 + 再 scrape):
  ...
  TASK_CARDS/14  SILVER 統合拡張 (8 カード並列、§10.2 / §12.1 を内包)
...
中期:
  ...
  TASK_CARDS/15  拡張目的レポート (8 カード + 横断タスク)

長期:
  TASK_CARDS/16  新ソース (LiveChart / TMDb) — §12.4
  TASK_CARDS/17  bangumi 日次差分 cron — §13.6 (dump 安定運用後)
