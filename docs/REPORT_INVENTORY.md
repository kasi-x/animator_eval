...
| O8 | soft_power | biz | policy | ✅ Tier1 実装済 (2026-05-02); Tier2 Card 16 |
...
|----|-------------|------------|----------|-------|----------|--------|
| O1 | o1_gender_ceiling.py | O1GenderCeilingReport | o1_gender_ceiling.html | ジェンダー天井効果分析 | policy | ✅ 実装済 (2026-05-04) |
| O3 | o3_ip_dependency.py | O3IpDependencyReport | o3_ip_dependency.html | IP 人的依存リスク分析 | biz | ✅ 実装済 (2026-05-02) |
| O4 | o4_foreign_talent.py | O4ForeignTalentReport | o4_foreign_talent.html | 海外人材ポジション分析 | policy, biz | ✅ 実装済 (2026-05-02) |
| O8 | o8_soft_power.py | O8SoftPowerReport | o8_soft_power.html | ソフトパワー指標 (Tier1) | biz | ✅ Tier1 実装済 (2026-05-02) |

### O1 — ジェンダー天井効果分析

- **Method**: Cox PH 回帰 (gender_f covariate, cohort_5y 調整) + Mann-Whitney U (コホート内昇進タイミング差) + ego-network same_gender_share vs. permutation null model (1000 iter)
- **Brief 組込み**: `scripts/report_generators/briefs/policy_brief.py` section `gender_progression_disparity`
- **Hard constraints**: H1 遵守 (外部視聴者評価を寄与計算から完全除外)、H2 遵守 (lint_vocab clean, 0 violations)
- **Tests**: `tests/reports/test_o1_gender_ceiling.py` (22 tests: smoke / lint_vocab / method gate / analysis unit tests)
- **Note**: gender カバレッジ ~11.5% (前回 null 88.5%) — pre-condition 閾値 70% を超えるが実用的 sample (女性 10,619 + 男性 20,560) で分析実施
...
### O4 — 海外人材ポジション分析 (完了 2026-05-02)

- **Method**: nationality_resolver (country_of_origin high → name_zh/ko medium → unknown low) + 国籍別 person FE 分布 violin + Mann-Whitney U (各グループ vs JP) + 役職進行 KM curve 国籍層別 (Greenwood 95% CI, log-rank vs JP) + studio FE 帰属散布図 (Pearson r, bootstrap CI n=1000, seed=42)
- **Limited mobility bias**: Andrews et al. (2008) — 海外人材はスタジオ間移動が少なく person FE 推定精度が低下。method note に必須記載。
- **Brief 組込み**: policy_brief_index リンク追加 / biz_brief_index リンク追加
- **Hard constraints**: H1 遵守 (anime.score 不使用)、H2 遵守 (lint_vocab clean, 0 violations)、H3 遵守 (entity_resolution 不変)
- **meta_lineage_table**: `meta_o4_foreign_talent`
- **Tests**: `tests/reports/test_o4_foreign_talent.py` (smoke / lint_vocab / method gate / resolve_nationality / load_nationality_records / NationalitySummary / person_fe_by_nationality / studio_foreign_share)

### O8 — ソフトパワー指標 (Tier1 完了 2026-05-02)

- **Method (Tier1)**: AniList external_links_json からプラットフォームリンク抽出 → Mann-Whitney U (国際展開 vs 国内専 theta_proxy 分布比較, bootstrap CI n=1000, seed=42) → soft_power_index = anime_count × mean_theta_proxy (platform_weight 全プラットフォーム 1.0 固定)
- **Tier2 予定**: 国際賞受賞重み / 配信国数 / 海外売上比率 (Card 16)
- **Brief 組込み**: Business brief (biz_brief_index リンク追加)
- **Hard constraints**: H1 遵守 (anime.score・display_score 算出パスから完全除外)、H2 遵守 (lint_vocab clean, 0 violations)
- **meta_lineage_table**: `meta_o8_soft_power`
- **Tests**: `tests/reports/test_o8_soft_power.py` (smoke / platform extraction / Mann-Whitney U / soft_power_index formula / lint / method gate)
