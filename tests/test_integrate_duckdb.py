...
def test_persons_birth_date_mapped(bronze_dir: Path, tmp_path: Path) -> None:
    ...
    conn.execute("SET schema='conformed'")
    row = conn.execute(
        "SELECT birth_date, website_url FROM persons WHERE id='anilist:p1'"
    ).fetchone()
    conn.close()
...
def test_dedup_keeps_latest_date(tmp_path: Path) -> None:
        ...
        bw.append(
            {
                ...
                "end_date": None,
                "status": "FINISHED",
                "original_work_type": None,
                "source": None,
                "work_type": None,
                "scale_class": None,
                "fetched_at": "2026-04-22T12:00:00",
    ...
    conn.execute("SET schema='conformed'")
    row = conn.execute(
        "SELECT title_ja, episodes FROM anime WHERE id='anilist:1'"
    ).fetchone()
    conn.close()
...
def test_credits_dedup_on_unique_key(tmp_path: Path) -> None:
    ...
    with BronzeWriter(
        "seesaawiki", table="credits", root=root, date=_dt.date(2026, 4, 22)
    ) as bw:
        bw.append(dup_credit)
    with BronzeWriter(
        "seesaawiki", table="credits", root=root, date=_dt.date(2026, 4, 23)
    ) as bw:
        bw.append(dup_credit)
...
def test_studios_loaded_when_parquet_exists(tmp_path: Path) -> None:
    ...
    conn.execute("SET schema='conformed'")
    row = conn.execute(
        "SELECT name, is_animation_studio FROM studios WHERE id='anilist:s1'"
    ).fetchone()
    link = conn.execute(
        "SELECT is_main FROM anime_studios WHERE anime_id='a1'"
    ).fetchone()
    conn.close()
...
def test_studios_skipped_gracefully_when_no_parquet(
