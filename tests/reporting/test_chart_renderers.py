...
def test_scatter_basic() -> None:
    spec = ScatterSpec(
        slug="s1", title="T", data_key="d", explanation=_exp(), x_field="x", y_field="y"
    )
...
def test_scatter_empty_data() -> None:
    spec = ScatterSpec(
        slug="s4", title="T", data_key="d", explanation=_exp(), x_field="x", y_field="y"
    )
...
def test_forest_auto_height() -> None:
    ...
    spec = ForestSpec(
        slug="f2", title="T", data_key="d", explanation=_exp(), height=None
    )
...
def test_ridge_basic() -> None:
    ...
    data = {
        "d": [{"g": "A", "v": float(i)} for i in range(10)]
        + [{"g": "B", "v": float(i)} for i in range(10)]
    }
    fig = render_chart(spec, data)
    assert isinstance(fig, go.Figure)
...
def test_line_single_series() -> None:
    spec = LineSpec(
        slug="l1", title="T", data_key="d", explanation=_exp(), x_field="t", y_field="v"
    )
...
def test_histogram_basic() -> None:
    spec = HistogramSpec(
        slug="hi1", title="T", data_key="d", explanation=_exp(), value_field="v"
    )
