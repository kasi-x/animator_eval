...
from src.analysis.compatibility import compute_compatibility_groups, GroupCompatibilityResult
...
def anime_map():
    ...
    return {
        "a1": Anime(id="a1", title_en="Show A", year=2018, score=8.0, studios=["Studio1"]),
        "a2": Anime(id="a2", title_en="Show B", year=2019, score=7.5, studios=["Studio1"]),
        "a3": Anime(id="a3", title_en="Show C", year=2019, score=9.0, studios=["Studio2"]),
        "a4": Anime(id="a4", title_en="Show D", year=2020, score=6.5, studios=["Studio2"]),
        "a5": Anime(id="a5", title_en="Show E", year=2021, score=8.5, studios=["Studio1"]),
        "a6": Anime(id="a6", title_en="Show F", year=2021, score=7.0, studios=["Studio3"]),
    }
...
def credits_with_pairs():
    ...
    return [
        ...
        Credit(person_id="p2", anime_id="a1", role=Role.ANIMATION_DIRECTOR, source="test"),
        ...
        Credit(person_id="p2", anime_id="a5", role=Role.ANIMATION_DIRECTOR, source="test"),
...
def iv_scores():
    return {
        "p1": 70.0, "p2": 65.0, "p3": 80.0, "p4": 55.0,
        "p5": 45.0, "p6": 60.0, "p7": 50.0,
    }
...
class TestPairDetection:
    ...
    def test_return_type(self, credits_with_pairs, anime_map, iv_scores):
        result = compute_compatibility_groups(
            credits_with_pairs, anime_map, iv_scores
        )
    ...
    def test_pair_has_compatibility_score(self, credits_with_pairs, anime_map, iv_scores):
        result = compute_compatibility_groups(
            credits_with_pairs, anime_map, iv_scores
        )
        for pair in result.compatible_pairs:
...
class TestMinSharedWorks:
    ...
    def test_custom_min_shared_works(self, anime_map, iv_scores):
        ...
        credits = [
            ...
            Credit(person_id="p1", anime_id="a3", role=Role.KEY_ANIMATOR, source="test"),
            Credit(person_id="p2", anime_id="a4", role=Role.ANIMATION_DIRECTOR, source="test"),
        ]
        result = compute_compatibility_groups(
...
class TestBridgePersons:
    ...
    def test_bridge_person_detected(self, anime_map):
        ...
        credits = [
            ...
            Credit(person_id="p1", anime_id="a6", role=Role.KEY_ANIMATOR, source="test"),
            Credit(person_id="p3", anime_id="a6", role=Role.DIRECTOR, source="test"),
            ...
            Credit(person_id="p2", anime_id="a6", role=Role.ANIMATION_DIRECTOR, source="test"),
            Credit(person_id="p4", anime_id="a6", role=Role.KEY_ANIMATOR, source="test"),
