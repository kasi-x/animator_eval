...
def test_unchanged_pages_skipped(cache_dir, setup, tmp_path):
    ...
    with (
        patch(
            "src.scrapers.sakuga_atwiki_scraper.PlaywrightFetcher", return_value=fetcher
        ),
        patch(
            "src.scrapers.sakuga_atwiki_scraper.fetch_disallow_patterns",
            return_value=[],
        ),
    ):
        stats = _run(
...
def test_changed_page_triggers_write(cache_dir, setup, tmp_path):
    ...
    credits_dir = (
        tmp_path / "bronze" / "source=sakuga_atwiki" / "table=credits" / "date=20260425"
    )
...
def test_existing_parquet_not_overwritten(cache_dir, setup, tmp_path):
    old_dir = (
        tmp_path / "bronze" / "source=sakuga_atwiki" / "table=credits" / "date=20260101"
    )
