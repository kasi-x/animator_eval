...
2. insights_report.py — generate_comprehensive_insights and sub-analyses
3. path_finding.py — find_shortest_path, find_all_shortest_paths, find_bottleneck_nodes
4. temporal_influence.py — compute_temporal_profiles, analyze_cohort_trends, detect_industry_trends
5. genre_specialization.py — compute_genre_profiles, find_genre_specialists, compute_genre_similarity
6. structural_holes.py — compute_structural_hole_metrics, compute_brokerage_metrics, classify_brokerage_role
7. core_periphery.py — identify_core_periphery, compute_coreness_metrics, find_rising_stars
8. bias_detector.py — detect_role_bias, detect_studio_bias, detect_systematic_biases
9. compensation_analyzer.py — analyze_fair_compensation, compute_gini_coefficient, compute_fairness_metrics
"""
...
class TestInsightsReport:
    ...
    def test_analyze_pagerank_distribution_empty(self):
    ...
    def test_analyze_pagerank_distribution_normal(self):
        from src.analysis.insights_report import analyze_pagerank_distribution
    ...
    def test_analyze_bias_correction_empty(self):
        from src.analysis.insights_report import analyze_bias_correction_impact

        result = analyze_bias_correction_impact({}, {})
        assert result.total_persons_affected == 0

    def test_analyze_growth_patterns_empty(self):
        from src.analysis.insights_report import analyze_growth_patterns

        result = analyze_growth_patterns({}, {})
        assert result.rising_stars_count == 0
        assert result.avg_velocity == 0.0

    def test_analyze_growth_patterns_normal(self):
        from src.analysis.insights_report import analyze_growth_patterns
        ...
        growth_data = {
            "growth_metrics": {
                f"p{i}": {
                    ...
                    "growth_velocity": float(i),
                    "momentum_score": float(i * 0.5),
                    "career_years": i,
                }
                for i in range(1, 11)
            }
        }
        names = {f"p{i}": f"Person {i}" for i in range(1, 11)}
        result = analyze_growth_patterns(growth_data, names)
        assert result.rising_stars_count == 5
        assert result.stagnant_count == 5
    ...
    def test_generate_comprehensive_insights(self):
        ...
        assert result.pagerank is not None
        assert isinstance(result.recommendations, list)
        assert isinstance(result.key_findings, list)

    def test_identify_undervaluation_alerts(self):
        from src.analysis.insights_report import identify_undervaluation_alerts
...
# ===========================================================================
# 9. Compensation Analyzer
# ===========================================================================


class TestCompensationAnalyzer:
    def test_compute_gini_coefficient_equal(self):
        from src.analysis.compensation_analyzer import compute_gini_coefficient

        # All equal values -> Gini = 0
        gini = compute_gini_coefficient([10.0, 10.0, 10.0, 10.0])
        assert gini == 0.0

    def test_compute_gini_coefficient_empty(self):
        from src.analysis.compensation_analyzer import compute_gini_coefficient

        assert compute_gini_coefficient([]) == 0.0
        assert compute_gini_coefficient([5.0]) == 0.0

    def test_compute_gini_coefficient_unequal(self):
        from src.analysis.compensation_analyzer import compute_gini_coefficient

        # Equal distribution -> Gini = 0
        gini_equal = compute_gini_coefficient([10.0, 10.0, 10.0, 10.0])
        # Unequal distribution should differ from equal
        gini_unequal = compute_gini_coefficient([1.0, 1.0, 1.0, 100.0])
        assert gini_unequal != gini_equal

    def test_classify_anime_type_movie(self):
        from src.analysis.compensation_analyzer import AnimeType, classify_anime_type

        anime = _make_anime("a1", episodes=1, season=None)
        assert classify_anime_type(anime) == AnimeType.MOVIE

    def test_classify_anime_type_ova(self):
        from src.analysis.compensation_analyzer import AnimeType, classify_anime_type

        anime = _make_anime("a1", episodes=1, season="summer")
        assert classify_anime_type(anime) == AnimeType.OVA

    def test_classify_anime_type_tv_1cour(self):
        from src.analysis.compensation_analyzer import AnimeType, classify_anime_type

        anime = _make_anime("a1", episodes=12)
        assert classify_anime_type(anime) == AnimeType.TV_1COUR

    def test_classify_anime_type_tv_2cour(self):
        from src.analysis.compensation_analyzer import AnimeType, classify_anime_type

        anime = _make_anime("a1", episodes=24)
        assert classify_anime_type(anime) == AnimeType.TV_2COUR

    def test_classify_anime_type_tv_long(self):
        from src.analysis.compensation_analyzer import AnimeType, classify_anime_type

        anime = _make_anime("a1", episodes=50)
        assert classify_anime_type(anime) == AnimeType.TV_LONG

    def test_analyze_fair_compensation_empty_contributions(self):
        from src.analysis.compensation_analyzer import (
            CompensationAnalysisRequest,
            analyze_fair_compensation,
        )

        request = CompensationAnalysisRequest(anime_id="a1", total_budget=100.0)
        anime = _make_anime("a1")
        result = analyze_fair_compensation(request, {}, anime)
        assert result.allocations == {}
        assert result.fairness_metrics.gini_coefficient == 0.0

    def test_analyze_fair_compensation_normal(self):
        from src.analysis.compensation_analyzer import (
            CompensationAnalysisRequest,
            analyze_fair_compensation,
        )
        ...
        anime = _make_anime("a1", episodes=12)
        contributions = {
            "p1": {"shapley_value": 30.0, "role": "director"},
            "p2": {"shapley_value": 20.0, "role": "key_animator"},
            "p3": {"shapley_value": 10.0, "role": "in_between"},
        }
        result = analyze_fair_compensation(request, contributions, anime)
        assert len(result.allocations) == 3
        # Total allocations should approximately equal budget
        total_alloc = sum(result.allocations.values())
        assert total_alloc == pytest.approx(100.0, abs=1.0)

    def test_analyze_fair_compensation_max_ratio_cap(self):
        from src.analysis.compensation_analyzer import (
            CompensationAnalysisRequest,
            analyze_fair_compensation,
        )
        ...
        request = CompensationAnalysisRequest(
            anime_id="a1", total_budget=100.0, max_ratio=3.0
        )
        anime = _make_anime("a1", episodes=12)
        # Moderately skewed Shapley values with more staff
        contributions = {
            "p1": {"shapley_value": 40.0, "role": "director"},
            "p2": {"shapley_value": 30.0, "role": "key_animator"},
            "p3": {"shapley_value": 20.0, "role": "animation_director"},
            "p4": {"shapley_value": 10.0, "role": "in_between"},
        }
        result = analyze_fair_compensation(request, contributions, anime)
        max_comp = max(result.allocations.values())
        # After capping, the top person's allocation should be reduced
        # vs. uncapped proportional allocation
        uncapped_top = 40.0 / 100.0 * 100.0
        # The capped allocation should be less than or equal to uncapped
        assert max_comp <= uncapped_top + 1.0

    def test_compute_fairness_metrics_empty(self):
        from src.analysis.compensation_analyzer import compute_fairness_metrics

        metrics = compute_fairness_metrics({}, {})
        assert metrics.gini_coefficient == 0.0

    def test_batch_analyze_compensation(self):
        from src.analysis.compensation_analyzer import batch_analyze_compensation

        anime_list = [_make_anime("a1", episodes=12), _make_anime("a2", episodes=24)]
        all_contributions = {
            "a1": {
                "p1": {"shapley_value": 20.0, "role": "director"},
                "p2": {"shapley_value": 10.0, "role": "key_animator"},
            },
            "a2": {
        ...
        assert results["a2"].anime_type == "tv_2cour"
