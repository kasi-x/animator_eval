...
def _make_test_data():
    ...
    anime_map = {
        f"a{i}": Anime(id=f"a{i}", title_en=f"Anime {i}", year=2020 + i, score=7.0 + i * 0.2)
        for i in range(1, 6)
    }
...
class TestFindDirectorCircles:
    ...
    def test_min_director_works_filter(self):
        ...
        circles = find_director_circles(credits, anime_map, min_shared_works=2, min_director_works=4)
