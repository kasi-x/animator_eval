...
def anime_map():
    ...
    return {
        "a1": Anime(id="a1", title_en="Work A", year=2018, score=8.5, studios=["Alpha"]),
        "a2": Anime(id="a2", title_en="Work B", year=2019, score=7.0, studios=["Alpha"]),
        ...
        "a5": Anime(id="a5", title_en="Work E", year=2020, score=8.0, studios=["Alpha"]),
        "a6": Anime(id="a6", title_en="Work F", year=2021, score=7.5, studios=["Gamma"]),
        "a7": Anime(id="a7", title_en="Work G", year=2021, score=8.8, studios=["Alpha"]),
        "a8": Anime(id="a8", title_en="Work H", year=2022, score=6.0, studios=["Gamma"]),
    }
...
def credits():
    ...
    return [
        ...
        Credit(person_id="p5", anime_id="a6", role=Role.ANIMATION_DIRECTOR, source="test"),
        Credit(person_id="p5", anime_id="a8", role=Role.ANIMATION_DIRECTOR, source="test"),
        ...
        Credit(person_id="p7", anime_id="a1", role=Role.ANIMATION_DIRECTOR, source="test"),
        Credit(person_id="p7", anime_id="a6", role=Role.ANIMATION_DIRECTOR, source="test"),
...
def person_fe():
    ...
    return {
        "d1": 2.5, "d2": 1.8, "d3": 1.2, "d4": 0.5,
        "p1": 2.0, "p2": 1.5, "p3": 0.8, "p4": 1.9,
        "p5": 1.1, "p6": 1.3, "p7": 0.9, "p8": 0.6,
    }
...
def birank():
    ...
    return {
        "d1": 0.85, "d2": 0.70, "d3": 0.55, "d4": 0.30,
        "p1": 0.40, "p2": 0.35, "p3": 0.25, "p4": 0.45,
        "p5": 0.30, "p6": 0.28, "p7": 0.22, "p8": 0.15,
    }
...
def iv_scores():
    ...
    return {
        "d1": 85.0, "d2": 70.0, "d3": 55.0, "d4": 30.0,
        "p1": 75.0, "p2": 60.0, "p3": 40.0, "p4": 72.0,
        "p5": 50.0, "p6": 48.0, "p7": 38.0, "p8": 25.0,
    }
...
class TestBasicComputation:
    ...
    def test_return_type(
        self, credits, anime_map, person_fe, birank, studio_fe,
        studio_assignments, iv_scores,
    ):
        result = compute_expected_ability(
            credits, anime_map, person_fe, birank, studio_fe,
            studio_assignments, iv_scores,
        )
...
class TestInsufficientData:
    ...
    def test_exactly_nine_persons(self, anime_map, studio_fe, studio_assignments):
        ...
        credits = [
            Credit(person_id=f"px{i}", anime_id=f"a{(i % 8) + 1}", role=Role.KEY_ANIMATOR, source="test")
            for i in range(1, 10)
        ]
