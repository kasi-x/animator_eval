...
class TestCharacterDiversity:
    ...
    def test_diverse_portfolio_higher_genre_entropy(self):
        ...
        anime_diverse = {
            f"a{i}": _anime(f"a{i}", genres=[diverse_genres[i]]) for i in range(8)
        }
        anime_special = {
            f"b{i}": _anime(f"b{i}", genres=[specialist_genres[i]]) for i in range(8)
        }
        char_map_d = {f"c{i}": _char(f"c{i}") for i in range(8)}
        char_map_s = {f"d{i}": _char(f"d{i}") for i in range(8)}

        credits_diverse = [
            _cva("va_diverse", f"a{i}", f"c{i}", "MAIN") for i in range(8)
        ]
        credits_special = [_cva("va_spec", f"b{i}", f"d{i}", "MAIN") for i in range(8)]

        result_d = compute_character_diversity(
            credits_diverse, anime_diverse, char_map_d
        )
        result_s = compute_character_diversity(
            credits_special, anime_special, char_map_s
        )

        if "va_diverse" in result_d and "va_spec" in result_s:
            assert (
                result_d["va_diverse"].genre_entropy
                >= result_s["va_spec"].genre_entropy
            )
...
class TestVaTrust:
    ...
    def test_va_with_sound_director_gets_trust(self):
        ...
        result = compute_va_trust(
            va_credits, prod_credits, anime_map, current_year=2025
        )
        assert "va1" in result
...
class TestReplacementDifficulty:
    ...
    def test_rdi_between_0_and_1(self):
        ...
        anime_map = {
            f"a{i}": _anime(f"a{i}", genres=[f"Genre{i % 3}"]) for i in range(8)
        }
...
class TestEnsembleSynergy:
    ...
    def test_pair_above_min_shared_included(self):
        ...
        va_credits = [_cva("va1", f"a{i}", f"c1_{i}", "MAIN") for i in range(4)] + [
