...
class TestCoverageRow:
    def test_bronze_studio_pct(self) -> None:
        row = CoverageRow(
            "test", bronze_anime=100, bronze_with_studio=60, silver_in_anime_studios=30
        )
    ...
    def test_zero_bronze_anime(self) -> None:
        row = CoverageRow(
            "test", bronze_anime=0, bronze_with_studio=0, silver_in_anime_studios=0
        )
    ...
    def test_capture_rate_capped_at_one(self) -> None:
        ...
        row = CoverageRow(
            "test", bronze_anime=10, bronze_with_studio=5, silver_in_anime_studios=10
        )
...
class TestMadbAnimationStudios:
    ...
    def bronze_root(self, tmp_path: Path) -> Path:
        ...
        def _write(table_name: str, schema: pa.Schema, data: dict) -> None:
            date_dir = (
                root / "source=mediaarts" / f"table={table_name}" / "date=2026-04-27"
            )
        ...
        _write(
            "broadcast_schedule",
            pa.schema(
                [pa.field("madb_id", pa.string()), pa.field("raw_text", pa.string())]
            ),
            {"madb_id": [], "raw_text": []},
        )
        _write(
    ...
    def test_animation_studio_in_anime_studios(self, bronze_root: Path) -> None:
    ...
    def test_animation_studio_id_format(self, bronze_root: Path) -> None:
    ...
    def test_producer_is_main_true_also_included(self, bronze_root: Path) -> None:
    ...
    def test_non_main_production_included_as_support(self, bronze_root: Path) -> None:
