...
class TestDedupBasic:
    def test_empty_db_returns_zero_deleted(
    ...
    def test_deletes_expected_count(
        self, conn_with_dups: duckdb.DuckDBPyConnection
    ) -> None:
        # Before: 9 rows total
    ...
    def test_cross_source_preserved(
        ...
        assert sources == {"anilist", "bangumi"}, (
            f"expected both sources, got {sources}"
        )

    def test_null_person_dups_removed(
        self, conn_null_person_dups: duckdb.DuckDBPyConnection
    ) -> None:
        before = conn_null_person_dups.execute(
...
class TestDedupIdempotent:
    def test_second_call_deletes_zero(
        self, conn_with_dups: duckdb.DuckDBPyConnection
    ) -> None:
        dedup(conn_with_dups, dry_run=False)
    ...
    def test_second_call_row_count_stable(
        self, conn_with_dups: duckdb.DuckDBPyConnection
    ) -> None:
...
class TestDedupDryRun:
    ...
    def test_dry_run_reports_correct_to_delete(
        self, conn_with_dups: duckdb.DuckDBPyConnection
    ) -> None:
        stats = dedup(conn_with_dups, dry_run=True)
    ...
    def test_dry_run_before_equals_after(
        self, conn_with_dups: duckdb.DuckDBPyConnection
    ) -> None:
        stats = dedup(conn_with_dups, dry_run=True)
...
class TestDedupPerSource:
    ...
    def test_null_person_per_source(
        self, conn_null_person_dups: duckdb.DuckDBPyConnection
    ) -> None:
...
class TestH4Guard:
    def test_evidence_source_present_after_dedup(
        self, conn_with_dups: duckdb.DuckDBPyConnection
    ) -> None:
        dedup(conn_with_dups, dry_run=False)
        ...
        assert rows_with_null_source == 0, (
            "evidence_source must not be NULL on surviving rows"
        )

    def test_evidence_source_values_preserved(
        self, conn_with_dups: duckdb.DuckDBPyConnection
    ) -> None:
        dedup(conn_with_dups, dry_run=False)
        sources = {
            r[0]
            for r in conn_with_dups.execute(
