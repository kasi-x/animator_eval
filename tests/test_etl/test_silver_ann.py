...
class TestApplyDdl:
    ...
    def test_anime_extension_columns_created(self, tmp_path: Path) -> None:
        ...
        for col in (
            ...
            "ending_themes_json",
            "insert_songs_json",
            "official_websites_json",
            "vintage_raw",
            "image_url",
            "display_rating_votes",
            "display_rating_weighted",
            "display_rating_bayesian",
        ):
...
class TestAnimeInsert:
    ...
    def test_inserts_multiple_anime_rows(self, tmp_path: Path) -> None:
        ...
        n = conn.execute("SELECT COUNT(*) FROM anime WHERE id LIKE 'ann:%'").fetchone()[
            0
        ]
        assert n == 2
    ...
    def test_id_prefix_is_ann_a(self, tmp_path: Path) -> None:
        conn = _silver_conn(tmp_path)
        _write_bronze(
            tmp_path, "ann", "anime", [{"ann_id": 99, "title_en": "X", "title_ja": ""}]
        )
        integrate(conn, tmp_path)
    ...
    def test_counts_key_anime_ann_returned(self, tmp_path: Path) -> None:
        conn = _silver_conn(tmp_path)
        _write_bronze(
            tmp_path, "ann", "anime", [{"ann_id": 7, "title_en": "Z", "title_ja": ""}]
        )
        counts = integrate(conn, tmp_path)
    ...
    def test_no_anime_insert_error_key(self, tmp_path: Path) -> None:
        ...
        _write_bronze(
            ...
            "ann",
            "anime",
            [{"ann_id": 3, "title_en": "Clean", "title_ja": ""}],
        )
        counts = integrate(conn, tmp_path)
    ...
    def test_existing_row_not_overwritten(self, tmp_path: Path) -> None:
        ...
        conn = _silver_conn(tmp_path)
        conn.execute(
            "INSERT INTO anime (id, title_en, title_ja) VALUES ('ann:a1', 'Pre-existing', '')"
        )
        _write_bronze(
            tmp_path,
            "ann",
            ...
            [
                {
                    ...
                    "datetime": "2024-01-15",
                    "title": "Announcement",
                    "href": "https://ann.com/news/1",
                },
                {
                    "ann_anime_id": 1,
                    "datetime": "2024-02-01",
                    "title": "Cast Reveal",
                    "href": "https://ann.com/news/2",
                },
            ],
        )
        counts = integrate(conn, tmp_path)
...
class TestRelated:
    ...
    def test_relation_type_filled(self, tmp_path: Path) -> None:
        ...
        integrate(conn, tmp_path)
        row = conn.execute(
            "SELECT relation_type FROM anime_relations LIMIT 1"
        ).fetchone()
...
class TestIntegrateReturnValue:
    def test_returns_all_count_keys(self, tmp_path: Path) -> None:
        ...
        for table, row in [
            ...
            (
                "company",
                {"ann_anime_id": 1, "company_name": "C", "task": "T", "company_id": 1},
            ),
            (
                "releases",
                {
...
class TestAnnSuffixColumns:
    ...
    def test_ann_suffix_columns_populated_on_update(self, tmp_path: Path) -> None:
        ...
        conn = _silver_conn(tmp_path)
        conn.execute(
            "INSERT INTO anime (id, title_en) VALUES ('ann:a10', 'Suffix Test')"
        )
        ...
        assert (
            abs(row[1] - 8.2) < 0.001
        )  # display_rating_avg_ann ← display_rating_weighted
    ...
    def test_ann_suffix_columns_null_safe(self, tmp_path: Path) -> None:
        ...
        conn = _silver_conn(tmp_path)
        conn.execute(
            "INSERT INTO anime (id, title_en) VALUES ('ann:a20', 'NullRating')"
        )
