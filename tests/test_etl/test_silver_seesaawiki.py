...
class TestIntegrateGrossStudios:
    ...
    def test_gross_studios_dedup(self, tmp_path: Path) -> None:
        ...
        integrate(conn, tmp_path)
        assert (
            conn.execute("SELECT COUNT(*) FROM anime_gross_studios").fetchone()[0] == 1
        )
...
class TestIntegrateOriginalWorkInfo:
    ...
    def test_original_work_info_dedup(self, tmp_path: Path) -> None:
        ...
        integrate(conn, tmp_path)
        assert (
            conn.execute("SELECT COUNT(*) FROM anime_original_work_info").fetchone()[0]
            == 1
        )
...
class TestIntegratePersonsExtras:
    def test_persons_extras_filled(self, tmp_path: Path) -> None:
        ...
        conn.execute(
            "INSERT INTO persons (id, name_ja) VALUES ('seesaa:p1', '山田太郎')"
        )
        _write_parquet(
            tmp_path,
            "persons",
    ...
    def test_persons_extras_coalesce_preserves_existing(self, tmp_path: Path) -> None:
        ...
        conn = _make_conn()
        conn.execute(
            "INSERT INTO persons (id, name_ja) VALUES ('seesaa:p2', '佐藤花子')"
        )
        # Manually set description
        conn.execute("ALTER TABLE persons ADD COLUMN IF NOT EXISTS description TEXT")
        conn.execute(
