...
SPEC = make_default_spec(
    ...
    identifying_assumption=(
        ...
        '異なる選択での結果は感度分析 (sensitivity_grid 経由) で別 report に提示。'
        '更新は git log と config.py 経由でトレース、本レポートは snapshot 時点の '
        'parameter set を documentation する役割に限定する。'
    ),
    null_model=['N6'],
    ...
    sensitivity_grid=[
        SensitivityAxis(name='lambda_set', values=['default', 'data_driven', 'literature']),
        SensitivityAxis(name='role_weight_def', values=['equal', 'production_scale_weighted']),
        SensitivityAxis(name='dormancy_form', values=['linear', 'exponential', 'piecewise']),
    ],
    extra_limitations=[
        'パラメータは選択 — 「最適値」ではない',
    ...
    alternative_interpretations=(
        'λ 重み固定は理論的根拠ではなく初期 operational 選択。data-driven 再校正で結果が変動する可能性があると考えられる。',
        '役職 weight が equal でなく production_scale_weighted の場合、IV 順位が再編される可能性。',
    ),
)
