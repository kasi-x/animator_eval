...
SPEC = make_default_spec(
    ...
    identifying_assumption=(
        ...
        '実際の記録残存率の推定ではない。古作品 (1970 年代以前) の名前解決 precision '
        'は ~70% と低く、source 別 coverage のばらつきも大きいため、sensitivity_grid '
        'で source 集合と name resolution tier を切替え結論を確認する。'
    ),
    null_model=['N5'],
    ...
    ci_estimator='wilson',
    sensitivity_grid=[
        SensitivityAxis(name='source_set', values=['MADB_only', 'MADB+seesaa', 'all_5']),
        SensitivityAxis(name='name_resolution_tier', values=['exact', 'tier_1_4', 'all_5']),
        SensitivityAxis(name='decade_bin', values=['decade', '5y', '20y']),
    ],
    extra_limitations=[
    ...
    alternative_interpretations=(
        '年代別 record-density 上昇曲線は実際の記録復元ではなく source coverage の年代偏り (1990s 以降への偏向) を反映している可能性。decade-balanced sub-sample で再評価要。',
        '古作品の未復元ギャップは名前解決失敗 (precision ~70%) を含むため、entity resolution 改善後に density が大幅変動する可能性。tier 別 stratified 解析推奨。',
        'crowdsourced source (SeesaaWiki) の記載品質変動で、特定年代の density が artificial に高低し、構造的記録残存率と乖離している可能性がある。',
    ),
)
