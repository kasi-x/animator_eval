...
# v3: CB-safe gender mapping from src.viz.palettes (cross-report consistent)
from src.viz import palettes as _v3_pal  # noqa: E402

_GENDER_COLORS = {
    "Male": _v3_pal.GENDER["M"],
    "Female": _v3_pal.GENDER["F"],
    None: _v3_pal.GENDER["unknown"],
}
...
SPEC = make_default_spec(
    ...
    identifying_assumption=(
        ...
        'APC 同定問題に対し period FE / cohort FE のうちどれを固定するかで結論が '
        '変動するため、sensitivity_grid で各 spec を切替えた結果を併設する。'
    ),
    null_model=['N3', 'N5'],
    sources=['credits', 'persons', 'anime', 'person_scores'],
    ...
    sensitivity_grid=[
        SensitivityAxis(name='cohort_bin_width', values=['5y', '10y', '15y']),
        SensitivityAxis(name='apc_fix', values=['fix_period', 'fix_cohort', 'fix_age']),
        SensitivityAxis(name='visibility_gap_threshold', values=[2, 3, 5]),
    ],
    extra_limitations=[
    ...
    alternative_interpretations=(
        '世代別軌跡の差は cohort 固有要因ではなく period effect (景気循環 / streaming 拡大) を反映している可能性。period FE 固定 spec で再走推奨。',
        '若年 cohort の "上昇 trajectory" は累積観測期間が短いことの artifact であり、同一 career_year で truncate して比較すると差が消える可能性。',
        '可視性喪失 (visibility gap ≥ 3 年) の閾値は arbitrary であり、5 年に伸ばすと exit 率が下がり cohort 比較の結論が反転する可能性。sensitivity_grid で確認要。',
    ),
)
