...
class O2MidManagementReport(BaseReportGenerator):
    ...
    def _build_blockage_section(
        ...
        fig.add_vline(
            ...
            line_dash="dash",
            line_color="#a0a0a0",
            annotation_text="産業中央値",
            annotation_position="top right",
        )
        fig.update_layout(
            title="スタジオ別パイプライン詰まり (blockage_score, 95% CI bootstrap)",
            xaxis_title="blockage_score (年) = studio_median - industry_median",
            yaxis_title="",
            height=max(420, len(dedup_rows) * 22 + 120),
            margin=dict(l=160),
        )
        ...
        return ReportSection(
            title="スタジオ別パイプライン詰まり指標",
            findings_html=findings_html,
            visualization_html=plotly_div_safe(fig, "chart_studio_blockage", height=max(420, len(dedup_rows) * 22 + 120)),
            method_note=(
