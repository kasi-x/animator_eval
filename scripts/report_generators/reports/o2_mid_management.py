...
SPEC = make_default_spec(
    ...
    identifying_assumption=(
        ...
        'タイトル不変の実務拡大 (社内昇進) を捕捉できないため、credit-record 上の '
        '"進行停滞" が実際の career 停滞とは独立変動する可能性。sensitivity_grid '
        'で stage 境界と studio min_size を切替えて結論の robust 性を確認する。'
    ),
    null_model=['N4', 'N5'],
    ...
    sensitivity_grid=[
        SensitivityAxis(name='stage_boundary', values=['4_stage_default', '3_stage_merged', '5_stage_fine']),
        SensitivityAxis(name='studio_min_persons', values=[5, 10, 20]),
        SensitivityAxis(name='censoring_window_years', values=[3, 5, 7]),
    ],
    extra_limitations=[
    ...
    alternative_interpretations=(
        '作監段階で生存時間が長い observed pattern は実際の昇進停滞ではなく、credit-record で作監タイトルが長期に表記され続ける慣習を反映している可能性。社内昇進 (タイトル不変昇格) の補正後に再評価要。',
        'studio blockage_score の正値は当該スタジオの構造的問題ではなく、特定スタジオが大規模 TV 案件で長期 cycle を持つことの artifact である可能性。anime scale を control に入れた条件付き比較が望ましい。',
        'KM 中央値の差は censoring window 設定 (3 / 5 / 7 年) に強く依存し、設定変更で stage 間の relative ordering が変動する可能性がある。',
    ),
)
