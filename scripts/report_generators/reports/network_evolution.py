...
class NetworkEvolutionReport(BaseReportGenerator):
    ...
    def _build_entrant_rate_section(self, sb: SectionBuilder) -> ReportSection:
        ...
        return ReportSection(
            ...
            interpretation_html=(
                "<p>本稿の解釈: 近年に退出者の急増が見える現象は、右打ち切りの影響の一部であると考えられる。"
...
SPEC = make_default_spec(
    ...
    identifying_assumption=(
        '年次グラフは「その年の作品でのみ」観察される協業エッジを基にし、'
        '時代別の credit-record 粒度差 (1980s vs 2010s) で構造指標が上方バイアスを'
        '受ける可能性を明示。年次境界は事前固定、移動窓 / 累積定義は別 cut で扱う。'
        'cohort effect と secular trend の分離は本指標で不可能。'
    ),
    null_model=['N5'],
    ...
    sensitivity_grid=[
        SensitivityAxis(name='window_size', values=['1y', '3y', '5y']),
        SensitivityAxis(name='edge_weight_def', values=['count', 'log1p', 'min(role_weight)']),
    ],
    extra_limitations=[
    ...
    alternative_interpretations=(
        '構造指標の時代上昇は credit 記録粒度向上 (近年 source 充実) の人工的アーティファクト。secular なネットワーク拡大ではない可能性。',
        'クラスタリング係数の変動は industry consolidation (スタジオ統合) ではなく role taxonomy の精緻化に伴う共起頻度変化を反映する可能性。',
    ),
)
