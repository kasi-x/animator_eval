...
SPEC = make_default_spec(
    ...
    identifying_assumption=(
        ...
        'APC identification problem (age × period × cohort 完全分離不可) を踏まえ、'
        'period FE と cohort FE のうち固定する基準を sensitivity grid で切替え、'
        '結論の robust 性を別途確認する。'
    ),
    null_model=['N3', 'N5'],
    ...
    sensitivity_grid=[
        ...
        SensitivityAxis(name='age_period_constraint', values=['fix_period', 'fix_cohort', 'free']),
        SensitivityAxis(name='gender_imputation', values=['observed_only', 'observed_plus_inferred', 'all_unknown_dropped']),
    ],
    extra_limitations=[
    ...
    alternative_interpretations=(
        '世代間 trajectory 差は構造変化ではなく単なる period effect (景気循環 / 委員会方式の普及) を反映、cohort 固有の特徴を測ってない可能性。period FE 入れて再走要。',
        '若年 cohort の "上昇 trajectory" は累積観測時間が短いことの artifact であり、同一 career age で truncate すると差が消える可能性がある。',
        'gender 推定 88% coverage の残り 12% の unknown は特定 cohort (古い世代 / 海外 staff) に偏在しており、unknown を一律 "drop" すると見かけ上の性別構成に bias が入る可能性。',
    ),
)
