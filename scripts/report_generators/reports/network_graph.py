...
SPEC = make_default_spec(
    ...
    identifying_assumption=(
        ...
        'Modularity Q は resolution parameter γ に強く依存し、γ=1.0 が単一の '
        '"正しい" 分割を返さない。複数 seed の集約 (consensus clustering) で '
        'コミュニティ境界の安定性を別 spec で確認する。'
    ),
    null_model=['N1'],
    ...
    sensitivity_grid=[
        SensitivityAxis(name='top_n_cutoff', values=[200, 500, 1000]),
        SensitivityAxis(name='louvain_resolution', values=[0.8, 1.0, 1.5]),
        SensitivityAxis(name='layout_algo', values=['ForceAtlas2', 'spring', 'kamada_kawai']),
    ],
    extra_limitations=[
    ...
    alternative_interpretations=(
        '可視化されたコミュニティ境界は Louvain の確率的初期化 artifact である可能性。consensus clustering (複数 seed の majority vote) で安定境界のみ表示すべき。',
        '高 modularity Q は構造的コミュニティではなく per-anime cap (1 anime に N 人) による artificial cluster を反映している可能性。anime-bipartite reduction の妥当性を別途検証要。',
        'ForceAtlas2 の見かけ上のクラスタ距離は意味を持たないが、読者は "近い = 関連性高" と誤解する可能性。距離凡例を明示し、隣接行列との対応で再確認する必要がある。',
    ),
)
