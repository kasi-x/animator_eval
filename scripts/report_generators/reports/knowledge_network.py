...
SPEC = make_default_spec(
    ...
    identifying_assumption=(
        ...
        '理論モデル (homogeneous transmission) の仮定は実際の伝達非対称性を '
        '反映しないため、sensitivity grid で transmission spec を切替え結論の '
        'robust 性を確認する。Tier 境界は事前固定。'
    ),
    null_model=['N1', 'N2'],
    ...
    sensitivity_grid=[
        SensitivityAxis(name='tier_boundary', values=['equal_5', 'equal_10', 'kmeans_5']),
        SensitivityAxis(name='ndi_transmission_spec', values=['homogeneous', 'role_weighted', 'experience_weighted']),
        SensitivityAxis(name='mentoring_proxy', values=['shared_credit', 'stage_gap_3', 'stage_gap_5']),
    ],
    extra_limitations=[
    ...
    alternative_interpretations=(
        'AWCC 高値は知識伝達ハブではなく特定スタジオの常連 person を抽出している可能性。studio FE 入れた conditional centrality で再評価要。',
        'NDI null 外側性は network 機能ではなく degree distribution の long-tail を反映、N1/N2 の degree-preserving null では破れず弱い証拠の可能性。',
        'メンタリング proxy (共クレジット) は同等職位の協業も含むため、stage_gap ≥ 3 で再定義すると分布が大きく変動し、結論が反転する可能性がある。',
    ),
)
