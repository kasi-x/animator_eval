...
SPEC = make_default_spec(
    ...
    identifying_assumption=(
        ...
        'crowdsourced source (SeesaaWiki) の品質は時代別に変動する一方、'
        '公式 source (MADB) も coverage が不均一で、両者の "正解値" 仮定は '
        'sensitivity grid 経由でのみ調整可能。'
    ),
    null_model=['N6'],
    ...
    ci_estimator='wilson',
    sensitivity_grid=[
        SensitivityAxis(name='reference_source', values=['MADB', 'AniList', 'mixed']),
        SensitivityAxis(name='manual_verify_sample_n', values=[200, 500, 1000]),
        SensitivityAxis(name='name_resolution_tier', values=['exact', 'cross_source', 'all_5_tiers']),
    ],
    extra_limitations=[
    ...
    alternative_interpretations=(
        '低 coverage source は実際の網羅性ではなく entity resolution 失敗を反映している可能性。AI-assisted Tier 5 を含めた resolved 後の coverage と比較するべき。',
        '5 source の独立な "正解" を仮定するが、AniList / MAL は MADB から派生したエントリを含む可能性があり、source 間の独立性仮定が壊れている可能性。',
        '名前解決検証 sample (500 件) は active person 上位寄りに偏り、low-credit person の resolution 失敗率を過小評価している可能性。stratified sample で再評価要。',
    ),
)
