...
SPEC = make_default_spec(
    ...
    identifying_assumption=(
        ...
        'role_weight × scale の重み spec が contribution_share の絶対値を支配する '
        'ため、sensitivity_grid で別 weight scheme を試して順位安定性を確認する。'
        'relations_json の精度 (~5% 漏れ) で IP 境界が変動する。'
    ),
    null_model=['N4'],
    ...
    sensitivity_grid=[
        SensitivityAxis(name='role_weight_scheme', values=['default', 'flat', 'log_scale']),
        SensitivityAxis(name='ip_cluster_min_size', values=[2, 3, 5]),
        SensitivityAxis(name='counterfactual_method', values=['additive', 'leave_one_out', 'bootstrap_drop']),
    ],
    extra_limitations=[
    ...
    alternative_interpretations=(
        '高 contribution_share の IP は実際の依存度ではなく role_weight 設定で監督 / 作監に大重みを置いた spec の artifact である可能性。flat weight 仕様で再評価要。',
        'counterfactual_drop_pct は additive decomposition の前提 (寄与が線形分離可能) が壊れている場合に過大 / 過小評価する可能性。leave-one-out / bootstrap drop で再計算推奨。',
        'シリーズクラスタは relations_json 漏れ ~5% を含み、本来 1 IP の作品群が複数 cluster に分裂すると見かけ上の集中度が下がる可能性。manual review 上位 N IP で確認要。',
    ),
)
