...
class O3IpDependencyReport(BaseReportGenerator):
    ...
    def _build_counterfactual_section(
        ...
        centers = [r.counterfactual_drop_pct for r in top_results]
        lo_errs = [
            max(0.0, r.counterfactual_drop_pct - r.ci_lower / max(r.counterfactual_drop_pct * 1e-6 + 1, 1))
            for r in top_results
        ]
        hi_errs = [
            max(0.0, r.ci_upper / max(r.counterfactual_drop_pct * 1e-6 + 1, 1) - r.counterfactual_drop_pct)
            for r in top_results
        ]

        # Forest plot as horizontal scatter with error bars
        fig = go.Figure(
            go.Scatter(
                x=centers,
                y=labels,
                ...
                hovertemplate=(
                    ...
                    "drop_pct=%{x:.3f}<br>"
                    "null_percentile="
                    + str([f"{r.null_percentile:.1f}" for r in top_results])
                    + "<extra></extra>"
                ),
            )
        )
        fig.update_layout(
            title=(
                "key person 不在時の counterfactual 下落率（上位 "
                f"{len(top_results)} シリーズ; bootstrap 95% CI）"
            ),
            xaxis_title="counterfactual_drop_pct (0–1)",
            yaxis_title="person / シリーズ",
            height=max(380, len(labels) * 32 + 100),
