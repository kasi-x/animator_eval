...
class O1GenderCeilingReport(BaseReportGenerator):
    ...
    def _build_cox_section(
        ...
        pair_labels = [r.pair_label for r in cox_results]
        hr_vals = [r.hr_female_vs_male for r in cox_results]
        ci_lo = [r.ci_lower for r in cox_results]
        ci_hi = [r.ci_upper for r in cox_results]
        lo_errs = [max(0.0, hr - cl) for hr, cl in zip(hr_vals, ci_lo)]
        hi_errs = [max(0.0, cu - hr) for hr, cu in zip(hr_vals, ci_hi)]
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate=(
                "%{y}<br>"
                "HR(F vs M)=%{x:.3f}<extra></extra>"
            ),
        ))
        # Reference line at HR=1 (no difference)
        fig.add_vline(
            x=1.0,
            line_dash="dash",
            line_color="#a0a0a0",
            annotation_text="HR=1.0（差なし）",
            annotation_position="top right",
        )
        fig.update_layout(
            title=(
                "役職進行ハザード率の性別差 HR (F vs M, 95% CI) — Cox PH 回帰"
            ),
            xaxis_title="Hazard Ratio (F vs M)",
            yaxis_title="役職進行ペア",
            height=max(320, len(pair_labels) * 60 + 120),
