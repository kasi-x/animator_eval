...
SPEC = make_default_spec(
    ...
    identifying_assumption=(
        ...
        '少なく person FE 推定精度が低下する。unknown 国籍 (~30%) の取り扱いと '
        'name_zh-ko tier の閾値は結論に強く影響するため sensitivity_grid で確認する。'
    ),
    null_model=['N4', 'N5'],
    ...
    sensitivity_grid=[
        SensitivityAxis(name='nationality_tier', values=['high_only', 'high_medium', 'high_medium_unknown']),
        SensitivityAxis(name='name_zhko_threshold', values=[0.7, 0.85, 0.95]),
        SensitivityAxis(name='min_credits_per_person', values=[3, 10, 30]),
    ],
    extra_limitations=[
    ...
    alternative_interpretations=(
        '国籍別 person FE 差は実際の構造的位置差ではなく limited mobility bias (海外人材の studio 間移動が少なく FE 推定誤差が大) を反映している可能性。AKM 連結集合限定の robustness check 要。',
        'name_zh-ko tier の誤分類 (medium ~10-15%) で日本国籍 person が誤って海外 cluster に流入し、海外群の FE 平均を押し上げる artifact である可能性。tier 別 stratified 解析推奨。',
        '役職進行 KM の国籍別差は credit-record の visibility 差 (海外人材は specific source でのみ捕捉) を反映、実際の役職進行差ではない可能性がある。',
    ),
)
