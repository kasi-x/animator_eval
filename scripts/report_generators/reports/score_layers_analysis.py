...
SPEC = make_default_spec(
    ...
    claim=(
        'IV スコアの 3 層 (causal: theta_i + studio_exp / structural: birank + AWCC / '
        'patronage) 分解で各層の Pearson 相関 |r| < 0.7 を示し、層間直交性が '
        '一定程度確保されている (multicollinearity を抑制)'
    ),
    identifying_assumption=(
        '3 層の概念分離は事前定義であり、実際の data-driven 分離 (PCA / '
        'factor analysis) とは独立。直交性は線形相関のみで検証し、'
        '非線形依存 (mutual information) は捕捉外。'
    ),
    null_model=['N1', 'N6'],
    sources=['credits', 'persons', 'anime', 'feat_person_scores'],
    meta_table='meta_score_layers_analysis',
    estimator='Pearson r + Spearman rho on z-normalized 3 layers',
    ci_estimator='analytical_se',
    sensitivity_grid=[SensitivityAxis(name='正規化方法', values=['z-score', 'rank', 'log+z'])],
    extra_limitations=[
        '層間相関 |r| < 0.7 の閾値は事前固定、超過時の集約方法は別途検討',
        '線形相関のみで非線形依存は未検出',
        '個人 FE 推定の不確実性 (CI 幅) は層間相関に伝播',
    ],
)
