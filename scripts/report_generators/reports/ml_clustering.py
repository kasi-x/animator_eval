...
SPEC = make_default_spec(
    ...
    identifying_assumption=(
        ...
        'cluster 解釈は K と次元圧縮手法に強く依存し、sensitivity_grid で別 K / '
        '別圧縮手法 (UMAP / t-SNE) と比較し robust 性を確認する。'
    ),
    null_model=['N1', 'N6'],
    sources=['person_scores'],
    ...
    sensitivity_grid=[
        ...
        SensitivityAxis(name='dim_reduction', values=['PCA', 'UMAP', 'tSNE']),
        SensitivityAxis(name='feature_scaling', values=['standard', 'robust', 'minmax']),
    ],
    extra_limitations=[
    ...
    alternative_interpretations=(
        '5 cluster 構造は構造的グルーピングではなく K-Means の球形 cluster 仮定 + PCA 線形仮定が同時に効いた artifact である可能性。UMAP + HDBSCAN で非線形構造を許容すると別の cluster 数を返す可能性がある。',
        'silhouette ≥ 0.30 は中程度の分離度を示すが、Calinski-Harabasz / Davies-Bouldin では別 K が最適となる可能性。多指標 consensus で再評価要。',
        'PCA 第 1-3 主成分は credit_count / studio_fe / patronage の組み合わせを反映する一方、観測上の cluster は活動年数の長さで支配されている可能性。年齢 stratified clustering で確認推奨。',
    ),
)
