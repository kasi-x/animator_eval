...
class BridgeAnalysisReport(BaseReportGenerator):
    ...
    def _build_betweenness_distribution(self, sb: SectionBuilder) -> ReportSection:
        ...
        return ReportSection(
            ...
            interpretation_html=(
                "<p>本稿の解釈: 媒介中心性は、ネットワーク上の最短経路にどれだけ多く含まれるかを示す指標である。"
                "高い値を持つ人物は異なるコミュニティ間の情報・協業の「橋渡し」的位置にいると考えられる。"
    ...
    def _build_betweenness_deeper(self, sb: SectionBuilder) -> ReportSection:
        ...
        return ReportSection(
            ...
            interpretation_html=(
                "<p>本稿の解釈: 媒介中心性が高く次数中心性が低い人物（左上象限）は「構造的穴」を埋める"
                "ブリッジ型の位置にいると考えられる。一方、両方高い人物（右上）はハブ型である。"
    ...
    def _build_cross_studio_bridges(self, sb: SectionBuilder) -> ReportSection:
        ...
        return ReportSection(
            ...
            interpretation_html=(
                "<p>本稿の解釈: スタジオクラスタを横断する人物は、異なるスタジオ文化や制作手法の間を移動していると考えられる。"
                "横断クラスタ数が多い人物はフリーランスまたは業界内で幅広い経験を持つ。"
    ...
    def _build_cross_studio_deeper(self, sb: SectionBuilder) -> ReportSection:
        ...
        return ReportSection(
            ...
            interpretation_html=(
                "<p>本稿の解釈: 横断クラスタ数と媒介中心性に正の関係がある場合、スタジオ間を移動する人物が"
                "ネットワーク上でブリッジ的位置を占める傾向を示唆すると考えられる。ただし、因果の方向は"
                "不明確である。高い媒介中心性を持つ人物がスタジオ間移動の機会を得やすい"
                "（選抜効果）と、スタジオ間移動がネットワーク位置を向上させる"
                "（処遇効果）の両方が考えられる。</p>"
            ),
            section_id="cross_studio_deeper",
        )
    ...
    def _build_career_track_deeper(self, sb: SectionBuilder) -> ReportSection:
        ...
        return ReportSection(
            ...
            interpretation_html=(
                "<p>本稿の解釈: 特定のキャリアトラック（例: veteranやlate_bloomer）が高い媒介中心性を示す場合、"
                "長期のキャリアや転機がネットワーク上のブリッジ的位置と関連していると考えられる。"
                "ただし、キャリアトラックの分類自体がクレジット数・期間に基づくルールベースであるため、"
                "活動期間の長さが両方の指標に影響している交絡因子であると見られる。</p>"
            ),
            section_id="career_track_deeper",
        )
...
SPEC = make_default_spec(
    ...
    identifying_assumption=(
        'コミュニティ境界は Louvain modularity 最適化 — resolution パラメータ依存で、'
        'resolution=1.0 (default) を採用。Bridge は構造的位置の記述であり、機能的'
        '「橋渡し」役割の証明ではない。スタジオクラスタ横断は anime_studios 精度に依存し、'
        '35/01 / 19/01 cluster fix 完了後の Resolved 層を前提とする。null model は '
        'degree-preserving (Configuration Model) で外生性確保。'
    ),
    null_model=['N1', 'N2'],
    ...
    sensitivity_grid=[
        SensitivityAxis(name='louvain_resolution', values=[0.5, 1.0, 1.5, 2.0]),
        SensitivityAxis(name='community_algo', values=['louvain', 'label_prop', 'infomap']),
        SensitivityAxis(name='edge_weight', values=['raw', 'normalized', 'log1p']),
    ],
    extra_limitations=[
    ...
    alternative_interpretations=(
        'high bridge score は実際の橋渡し機能ではなく、複数 source で重複記録された hub person (e.g. プロデューサー兼監督) を反映する可能性。',
        'Louvain resolution=1.0 固定での結果。0.5 / 2.0 で community 数が変わり、結果として bridge 集合も変動する。',
    ),
)
