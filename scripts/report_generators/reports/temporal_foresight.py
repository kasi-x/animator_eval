...
SPEC = make_default_spec(
    ...
    identifying_assumption=(
        ...
        '将来軌跡の予測には別途 holdout 設計 (debut_year split) が必要。'
        'K-means の K=5 は ICA-like elbow analysis ではなく事前固定 — '
        '別 K での結果安定性は sensitivity_grid で別途確認すべき。'
    ),
    null_model=['N3'],
    ...
    holdout=HoldoutSpec(
        method='time-split',
        holdout_size='last 5 years (2020-2024) — concept only, 実装は未投入',
        metric='cluster assignment stability (ARI)',
        naive_baseline='random cluster assignment',
    ),
    sensitivity_grid=[
        SensitivityAxis(name='K', values=[3, 5, 7]),
        SensitivityAxis(name='feature_set', values=['credit_count', '+role_diversity', '+gap_pattern']),
        SensitivityAxis(name='clustering_algo', values=['kmeans', 'agglomerative', 'gmm']),
    ],
    extra_limitations=[
    ...
    alternative_interpretations=(
        '5 typology は K=5 固定の結果であり、K=3 / K=7 で異なる typology が出る可能性。クラスタ数は事前固定の operational 選択。',
        'typology が「キャリア軌跡」を反映するのではなく、データ source 別 credit 記録粒度差 (e.g. ANN は監督偏重) を反映している可能性。',
    ),
)
