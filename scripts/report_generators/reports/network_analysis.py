...
class NetworkAnalysisReport(BaseReportGenerator):
    ...
    def _build_hub_broker_deeper(self, sb: SectionBuilder) -> ReportSection:
        ...
        return ReportSection(
            ...
            interpretation_html=(
                "<p>本稿の解釈: 純粋ブリッジは少数の接続先を通じて多くの最短経路を持つ人物であり、"
                "ハブ兼ブリッジは多くの接続を持ちつつも橋渡し機能を果たす人物であると考えられる。"
    ...
    def _build_akm_mobility_deeper(self, sb: SectionBuilder) -> ReportSection:
        ...
        return ReportSection(
            ...
            interpretation_html=(
                "<p>本稿の解釈: 観測数が少ない人物（n &lt; 5）の person_fe 推定値は統計的に不安定であり、"
                "補償根拠として使用する際には信頼区間の明示が必要であると考えられる。"
                "観測数の分布はデータ収集の偏り（特定年代・スタジオの記録密度の差）を反映していると見られる。"
...
SPEC = make_default_spec(
    ...
    identifying_assumption=(
        ...
        '中心性は graph 構築時点 (snapshot) の構造的代理量であり、時間変動下の '
        '機能的影響は別 spec で対応。エッジ重み (role × episode × duration) の '
        '選択は感度分析で robust 性を確認する。'
    ),
    null_model=['N1', 'N2'],
    ...
    sensitivity_grid=[
        SensitivityAxis(name='edge_weight_spec', values=['role_only', 'role×episode', 'role×episode×duration']),
        SensitivityAxis(name='pagerank_damping', values=[0.75, 0.85, 0.95]),
        SensitivityAxis(name='betweenness_sample_k', values=[0, 100, 500]),
    ],
    extra_limitations=[
    ...
    alternative_interpretations=(
        '高中心性 person は構造的 hub ではなく特定 source の coverage 厚い person (主要スタジオの常連) を反映している可能性。source 別 sub-graph で再走するべき。',
        'configuration model null 外側性は network 機能ではなく degree distribution の long-tail を反映、null の選び方 (degree-preserving vs 完全 random) で結論が変動する可能性。',
        'weighted PageRank の damping factor 設定 (0.85) は arbitrary。0.75 / 0.95 で再計算するとランク順位が大きく変動する可能性がある。',
    ),
)
