...
class NetworkAnalysisReport(BaseReportGenerator):
    ...
    def _build_centrality_overview(self, sb: SectionBuilder) -> ReportSection:
        ...
        ec_vals = [r["eigenvector_centrality"] or 0.0 for r in rows]
        ...
        findings = (
            ...
            f"<p>キャリアトラック数: {len(tracks)} 種類。</p>"
        )

        # Violin plots for 3 metrics × tracks
        fig = go.Figure()
        metrics = [
            ("betweenness_centrality", "媒介中心性", bc_vals),
            ("degree_centrality", "次数中心性", dc_vals),
            ("eigenvector_centrality", "固有ベクトル中心性", ec_vals),
        ]
        # Build per-track per-metric violins using subplots simulation via one figure
        # Use separate violin traces per (track, metric) but group by metric for axis
        for track in tracks:
        ...
        return ReportSection(
            ...
            visualization_html=plotly_div_safe(fig, "chart_centrality_violin", height=480),
            method_note=(
    ...
    def _build_centrality_deeper(self, sb: SectionBuilder) -> ReportSection:
        ...
        findings = (
            ...
            f"{n_bridges:,} 名（全体の {n_bridges/len(rows)*100:.1f}%）。</p>"
        )
        ...
        fig = go.Figure()
        for track in tracks:
            mask = [r["career_track"] == track for r in rows]
            sub_rows = [r for r, m in zip(rows, mask) if m]
            if not sub_rows:
                continue
            sub_bc = [r["betweenness_centrality"] or 0.0 for r in sub_rows]
            sub_dc = [r["degree_centrality"] or 0.0 for r in sub_rows]
            sub_iv = [r["iv_score"] or 0.0 for r in sub_rows]
            sub_nc = [max(r["n_collaborators"] or 1, 1) for r in sub_rows]
            sub_names = [r["name"] for r in sub_rows]
        ...
        # Annotation lines for bridge quadrant
        ...
        return ReportSection(
            ...
            findings_html=findings,
            visualization_html=plotly_div_safe(fig, "chart_bc_dc_scatter", height=500),
            method_note=(
    ...
    def _build_hub_broker_overview(self, sb: SectionBuilder) -> ReportSection:
        ...
        tracks = [r["career_track"] for r in top30]
        ...
        colors = [color_map.get(t, "#aaaaaa") for t in tracks]
        ...
        return ReportSection(
            ...
            findings_html=findings,
            visualization_html=plotly_div_safe(fig, "chart_top_betweenness_bar", height=max(400, 18 * len(top30))),
            method_note=(
    ...
    def _build_hub_broker_deeper(self, sb: SectionBuilder) -> ReportSection:
        ...
        bc = [r["betweenness_centrality"] or 0.0 for r in rows]
        hs = [r["hub_score"] or 0.0 for r in rows]

        bc_arr = np.array(bc)
        hs_arr = np.array(hs)
        # "Pure brokers": high betweenness, low hub_score
        ...
        findings = (
            f"<p>媒介中心性（x軸）vs Hub Score（y軸）の散布図（{len(rows):,} 名サンプル）。</p>"
        ...
        fig.add_shape(type="line", x0=0, x1=1, y0=hs_50, y1=hs_50,
                      xref="paper", yref="y", line=dict(color="#667eea", dash="dot", width=1))
        fig.add_annotation(x=bc_75, y=max(hs), text="Betweenness 75th",
        ...
        return ReportSection(
            ...
            findings_html=findings,
            visualization_html=plotly_div_safe(fig, "chart_betweenness_hub_scatter", height=480),
            method_note=(
    ...
    def _build_tier_structure_overview(self, sb: SectionBuilder) -> ReportSection:
        ...
        # person → modal tier
        person_modal_tier: dict[str, str] = {
            pid: Counter(tiers).most_common(1)[0][0]
            for pid, tiers in person_tiers.items()
        }

        # Re-query degree_centrality per person (just need unique persons)
        ...
        findings = f"<p>作品スケール階層（modal）別の次数中心性分布（{len(pid_to_dc):,} 名）。</p>"
        findings += "<ul>"
        ...
        tiers_ordered = sorted(tier_dc.keys())
        fig = go.Figure()
        ...
        return ReportSection(
            ...
            findings_html=findings,
            visualization_html=plotly_div_safe(fig, "chart_tier_degree_box", height=440),
            method_note=(
    ...
    def _build_tier_structure_deeper(self, sb: SectionBuilder) -> ReportSection:
        ...
        # person → (modal_tier, career_track, mean_betweenness)
        ...
        # Aggregate: mean betweenness by (career_track, modal_tier)
        ...
        for track in tracks:
            row_vals = []
            for tier in tiers:
                vals = cell_vals.get((track, tier), [])
                row_vals.append(float(np.mean(vals)) if vals else 0.0)
            z.append(row_vals)

        findings = (
            f"<p>キャリアトラック（{len(tracks)} 種類）× スケール階層（{len(tiers)} 種類）の"
            f"セル別平均媒介中心性ヒートマップ（{len(person_data):,} 名）。</p>"
        )
        ...
        return ReportSection(
            ...
            findings_html=findings,
            visualization_html=plotly_div_safe(fig, "chart_track_tier_heatmap", height=360),
            method_note=(
    ...
    def _build_akm_mobility_overview(self, sb: SectionBuilder) -> ReportSection:
        ...
        # Correlation
        if len(rows) >= 10:
            corr = float(np.corrcoef(n_anime, person_fe)[0, 1])
        else:
            corr = float("nan")

        # Low mobility: n_unique_anime == 1
        n_single = int((n_anime == 1).sum())

        findings = (
            f"<p>AKMモビリティ診断: {len(rows):,} 名を対象に、"
            ...
            f"（全体の {n_single/len(rows)*100:.1f}%）。</p>"
        )

        # Sample for scatter (max 4000)
        ...
        return ReportSection(
            ...
            findings_html=findings,
            visualization_html=plotly_div_safe(fig, "chart_akm_mobility_scatter", height=460),
            method_note=(
    ...
    def _build_akm_mobility_deeper(self, sb: SectionBuilder) -> ReportSection:
        ...
        findings = (
            f"<p>AKM推定における観測数（person_fe_n_obs）の分布（{len(rows):,} 名）。</p>"
            f"<p>分布: {format_distribution_inline(summ)}、{format_ci((summ['ci_lower'], summ['ci_upper']))}。</p>"
            f"<p>観測数 &lt; 5（信頼性低）: {n_lt5:,} 名（{n_lt5/len(rows)*100:.1f}%）。</p>"
            f"<p>観測数 5–19: {n_5to20:,} 名（{n_5to20/len(rows)*100:.1f}%）。</p>"
            f"<p>観測数 ≥ 20（信頼性高）: {n_ge20:,} 名（{n_ge20/len(rows)*100:.1f}%）。</p>"
        )

        # Cap display at 100 for histogram readability
        cap = 100
        ...
        return ReportSection(
            ...
            findings_html=findings,
            visualization_html=plotly_div_safe(fig, "chart_akm_n_obs_hist", height=400),
            method_note=(
