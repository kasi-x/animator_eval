...
SPEC = make_default_spec(
    ...
    identifying_assumption=(
        ...
        'external_links_json に依存する。platform 集合と weight scheme で '
        'index の絶対値が変動するため、sensitivity_grid で robust 性を確認する。'
    ),
    null_model=['N3', 'N4'],
    ...
    sensitivity_grid=[
        SensitivityAxis(name='platform_set', values=['netflix_only', 'top_3', 'all_international']),
        SensitivityAxis(name='platform_weight', values=['flat_1.0', 'subscriber_share', 'launch_year_weighted']),
        SensitivityAxis(name='theta_proxy_spec', values=['network_only', 'akm_only', 'composite']),
    ],
    extra_limitations=[
    ...
    alternative_interpretations=(
        '国際配信作品の theta_proxy 高値は構造的人材集中ではなく "国際配信に選ばれる" selection effect (大型 IP / 大スタジオ作品が優先的に選定) を反映、配信プラットフォーム選定の bias を測ってる可能性。',
        'soft_power_index = count × mean_theta の multiplicative spec は anime_count に過敏で、count > 100 の outlier IP が index を支配する可能性。log spec での再計算推奨。',
        'AniList external_links_json の捕捉率に依存し、新規 platform (近年起ち上げ) の作品が含まれない場合、最新期間の soft_power が下方バイアスを受ける可能性がある。',
    ),
)
