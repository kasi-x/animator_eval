...
def plotly_div_safe(fig: go.Figure, div_id: str, height: int = 500) -> str:
    ...
    v3 (2026-05-05): theme は src.viz.theme.apply_theme("dark") に統一。
    box/violin の subsample (≤200点) は v3 でも維持 (HTML サイズ削減目的)。
    embed 本体は src.viz.embed.embed() に委譲。
    """
    # Local imports avoid circular dependency at module load time.
    from src.viz import embed as v3_embed
    from src.viz.theme import apply_theme

    apply_theme(fig, theme="dark", height=height)
    _subsample_box_violin_traces(fig)
    return v3_embed(fig, div_id, height=height)
